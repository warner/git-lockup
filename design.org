...
**** setup-assure contains an embedded key
**** Running setup-assure installs .git/TOOL (call it "assure-tool"), then
     runs "TOOL setup-subscribe"
**** "TOOL setup-subscribe" modifies .git/config to run "TOOL fetch" on fetch
*** publisher installs git-assure, runs "git-assure setup-publish"
**** that installs .git/TOOL
**** then runs "TOOL setup-suscribe" to perform setup-assure steps
**** then runs "TOOL setup-publish --create-keypair" for publish-specific
     parts: creates keypair, creates and git-adds setup-assure, advises to
     commit and push
