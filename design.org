...
** new name: "git-lockup"?
** setup-publish: git add the "setup-assure" and "assure.config" files
** setup.py build: create git-assure in a mktmp file, not build/temp, it's
   kind of in the way for tab-completion of build/[TAB]
...
* new design:
** first publisher has copy of git-assure, runs "git assure setup-publish"
*** that has a default behavior of --create-keypair=master
*** it copies git-assure into .git/git-assure verbatim
*** then creates a keypair, updates .git/config, adds post-commit hook
*** then creates ./setup-assure for clients, which include a copy of
    git-assure
*** then creates assure.config with the branch info
** post-commit hook runs .git/git-assure post-commit-hook, which signs
** subscribers run ./setup-assure
*** that dumps .git/git-assure
*** then runs .git/git-assure setup-client
**** which reads assure.config, modifies .git/config:
***** set remote.NAME.url to ext::.git/assure-tool fetch REMOTE RAWURL
***** set remote.NAME.pushurl (to either RAWURL or a pre-existing pushurl)
***** add branch.NAME.assure-key if not already set
